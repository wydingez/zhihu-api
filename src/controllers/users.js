...
class UsersCtrl {
  ...
  async collectAnswer (ctx, next) {
    ...
    if (!me.collectingAnswers.map(i => i.toString()).includes(ctx.params.id)) {
  ...
  async uncollectAnswer (ctx) {
    const me = await User.findById(ctx.state.user._id).select('+collectingAnswers')
    const index = me.collectingAnswers.map(i => i.toString()).indexOf(ctx.params.id)
    if (index > -1) {
  ...
  async listCollecttingAnswers (ctx) {
